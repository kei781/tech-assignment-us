...
원인은 **파일시스템에 DB가 제공하는 두 가지가 없다**는 데 있습니다. ① 여러 단계를 묶는 트랜잭션 — 원자적인 것은 `rename`·`link`·`O_EXCL` 생성 같은 단일 syscall 하나뿐입니다. ② 세션에 묶인 잠금 — DB는 연결이 끊기면 락을 해제하지만, 파일 잠금은 소유자의 생존을 알 수 없습니다. ②를 대체하려면 heartbeat·registry·stale timeout·리더 선출이 필요해지고, ①이 없어 그 회수 절차를 안전하게 만들 수 없습니다. 마지막 시도에서는 정확성을 잠금에서 버전 CAS(`fs.link`로 `versions/v{N+1}.json` 생성)로 옮겼는데, 이는 사실상 MVCC를 직접 구현하기 시작한 것이었습니다.

7차에 걸친 적대적 검증에서 **22건의 결함**이 나왔고, 그 시점에 명세는 700줄·요구사항 60여 개에 이르렀습니다.

또한 과제 내용에는 worker나 producer가 다중 프로세스로 동작해야 한다는 언급이 없습니다. **문구도 모두 복수가 아닌 단수입니다.**

| 원문 | |
|---|---|
| "**스케줄러**: `@nestjs/schedule`" | 단수 |
| "API 요청과 **스케줄러가** 동시에 같은 데이터에 접근할 수 있는 환경입니다" | 단수 |
| "**백그라운드 작업**을 통해 작업을 주기적으로 처리합니다" | 단수 |
| "데이터는 **단일** JSON 파일에 영속화합니다" | — |

`여러 프로세스`·`worker`·`producer`·`인스턴스`·`다중`에 해당하는 표현은 나오지 않습니다. 다중 프로세스는 제가 추가한 가정이었습니다.

**그래서 잠금을 더 정교하게 만드는 대신 간소화하여 문제를 해결하기로 결정했습니다.** writer를 프로세스 하나로 되돌리면 트랜잭션은 인메모리 mutex로, 생존 감지는 기동 복구 한 규칙으로 해결됩니다. 잠금 파일·Reaper·lease·CAS·설정 fingerprint가 모두 사라지고 규칙 다섯 개가 남았습니다. 전환 시점에 명세는 700줄·요구사항 60여 개에서 475줄·49개로 줄었습니다.

이후 적대적 검증을 반복하며 명세는 534줄·54개까지 다시 늘었습니다. 다만 늘어난 것은 **영속 데이터 검증과 오류 경계에 관한 규칙**이고, 잠금·생존 감지·리더 선출 기계는 돌아오지 않았습니다.

기능을 축소한 것은 아닙니다. 철회한 설계도 결국 한 번에 Job 하나를 처리했고, 파일 하나를 공유 저장소로 쓰는 한 병목은 그 파일이었습니다. 프로세스를 늘려도 실효 처리량은 늘지 않으면서 정확성만 잃는 구조였습니다.
