...
181개 테스트 / 7개 스위트입니다. 실제 스케줄러 주기나 처리 시간을 기다리지 않습니다([TST-002]) — 시계와 처리 로직을 주입하고 tick을 직접 호출합니다.
...
길이는 **코드포인트**로 셉니다 — 이모지 하나는 1자입니다(`String.length`로는 2). DTO와 로드 검증이 같은 함수를 쓰므로 두 경계의 판정이 어긋나지 않습니다.
...
| 도메인 | 상태·존재 (404 / 409) | `jobs.service.ts` (mutex 안) |
...
**시각은 형식만 봐서는 부족합니다.** `2026-02-30T00:00:00.000Z`은 정규식을 통과하고, JS `Date`가 이걸 `2026-03-02`로 넘겨버립니다. 오류도 안 나고 값만 바뀝니다. 그래서 `new Date(v).toISOString() === v`로 왕복시켜 실재 여부를 확인합니다.
...
```text
class-validator의 @MaxLength(1000)  →  코드포인트로 센다
내 로더의 String.length             →  UTF-16으로 센다

'😀'.repeat(600)  →  코드포인트 600 / UTF-16 1200
  DTO:  통과 (201로 저장됨)
  로더: 거부 → 다음 기동에서 부팅 실패
```
...
정의를 맞추는 것으로 끝내지 않았습니다. 라이브러리가 내부 계수 방식을 바꾸면 또 어긋나므로, `countCharacters`를 만들어 **DTO와 로더가 같은 함수를 부르게** 했습니다(`@MaxLength` → 직접 만든 `@MaxCharacters`).
