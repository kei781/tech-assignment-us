...
182개 테스트 / 7개 스위트입니다. 실제 스케줄러 주기나 처리 시간을 기다리지 않습니다([TST-002]) — 시계와 처리 로직을 주입하고 tick을 직접 호출합니다.

| 스위트 | 검증 대상 |
|---|---|
| `test/api.e2e-spec.ts` | 전체 엔드포인트 × 성공/실패, 요청 로깅, 부트스트랩, 저장→재기동 왕복 |
| `test/jobs-store.spec.ts` | 직렬화·원자적 저장·기동 복구·로드 시 스키마 검증 |
...
길이는 **코드포인트 수**로 셉니다. `String.length`(UTF-16 코드유닛)와 달리 `😀` 같은 단일 코드포인트 이모지는 1자입니다.

> 화면상 한 글자와는 다릅니다 — `❤️`(하트 + variation selector)는 2, `🇰🇷`(regional indicator 2개)는 2, `👨‍👩‍👧‍👦`(ZWJ 가족)는 7로 셉니다. grapheme cluster 단위로 세려면 `Intl.Segmenter`가 필요한데, 과제 범위에서는 코드포인트로 계약을 고정하고 **DTO와 로드 검증이 같은 함수를 쓰도록** 하는 것이 더 중요하다고 판단했습니다.
...
과제가 요구한 것은 **"API 요청과 스케줄러가 동시에 같은 데이터에 접근하는 환경에서 데이터가 손실되거나 깨지지 않게 하라"** 입니다. 이 문제를 다섯 개 규칙으로 나눠 풀었습니다.
...
| 손으로 고친 파일이 런타임에서 터진다 | **로드 시 파일 전체를 스키마 검증**하고, 어긋나면 원본을 보존한 채 기동 중단 |

#### 단일 스레드인데 왜 mutex가 필요한가

Node.js는 단일 스레드지만 이벤트루프, 논블로킹 io, 비동기적 처리의 특성때문에 **`await` 지점에서 실행이 교차합니다.**
...
`jobs.json`을 쓰는 모든 경로가 하나의 promise chain을 통과하게 만들면 이 순서가 성립하지 않습니다. 별도 라이브러리 없이 열몇 줄이면 됩니다.
...
**체인을 지키는 것은 흡수 단계입니다.** 앞선 변경이 실패해도 체인이 rejected로 굳지 않아야 이후 변경이 계속 실행됩니다. 실패는 `result`를 통해 **그 변경의 호출자에게만** 전달되고, 뒤에 줄 서 있는 다른 변경에는 번지지 않습니다. 흡수를 한쪽만 연결하면 첫 실패 이후 모든 변경이 영구히 멈춥니다.

처음에는 `this.mutexChain.then(task, task)`로 위쪽에도 실패 슬롯을 뒀는데, 흡수 단계가 체인을 항상 fulfilled로 만들기 때문에 그 슬롯은 **도달할 수 없는 코드**였습니다. 네 조합을 실측해 확인한 뒤 제거했습니다 — 방어가 두 겹인 것처럼 보이지만 실제로 일하는 것은 한 겹이었고, 주석이 일하지 않는 쪽을 가리키고 있었습니다.

> 같은 promise chain 패턴을 `FileLogger`도 씁니다. 다만 거기는 체인 자체를 결과로 덮으므로(`appendChain = appendChain.then(A, B)`) 흡수 전용 단계가 없고, **실패 슬롯이 체인을 지키는 유일한 수단**입니다. 구조가 다르면 결론도 달라집니다.
...
#### 신뢰 경계가 셋이라는 것 — 가장 비싸게 배운 부분

처음에는 검증을 **API 입력에만** 뒀습니다. DTO가 형식·길이·타입을 막으니 충분하다고 봤습니다. 그런데 `jobs.json`은 **사람이 직접 고칠 수 있는 파일**입니다.

```text
jobs.json에 {"jobs":[{}]} 를 넣으면
  기동:      성공
  GET /jobs: 200        ← 쓰레기가 클라이언트까지 나간다
  검색:      500        TypeError: Cannot read properties of undefined
```

`job.title`이 `undefined`인데 `.toLowerCase()`를 부른 것입니다. **원인(파일)과 증상(검색 500)이 멀어서** 디버깅이 어렵습니다. 그래서 검증 지점을 세 곳으로 나눴습니다.

| 경계 | 무엇을 보는가 | 어디 |
|---|---|---|
| API 입력 | 형식·타입·길이·정의되지 않은 필드 | `jobs.dto.ts` |
| 도메인 | 변경 시 상태·존재 (404 / 409) | `jobs.service.ts` — `mutate` 안 |
| 영속 데이터 | 파일 전체 스키마 | `jobs.types.ts` → 로드 시 |

로드 검증은 최상위 키가 정확히 `jobs` 하나인지, 각 레코드가 UUID v4 `id`·trim된 문자열·길이·`status` enum·**실재하는** ISO 8601 UTC 시각·허용된 키만 가졌는지, `id` 중복이 없는지를 봅니다. 하나라도 어긋나면 손상 파일과 같은 취급으로 **원본을 보존한 채** 기동을 멈춥니다.

두 가지가 특히 미묘했습니다.

**최상위 키를 관용적으로 무시하면 데이터가 사라집니다.** `{"jobs":[], "sentinel":{...}}`을 받아들이면 인메모리 상태가 `{ jobs }`만 담으므로 다음 쓰기에서 `sentinel`이 조용히 삭제됩니다. 파싱 실패는 파일을 지키면서 멈추는데 최상위 키는 지워버리면 **데이터 보호 방침에 일관성이 없습니다.**

**시각은 형식만 봐서는 부족합니다.** `2026-02-30T00:00:00.000Z`은 정규식을 통과하므로, 이전 로더는 이 값을 **그대로 승인해 보관**했습니다. 게다가 단순히 `new Date(v)`로 파싱하면 JS가 이걸 `2026-03-02`로 정규화해버려서, 파싱 결과를 쓰는 코드가 조용히 다른 날짜를 보게 됩니다. 그래서 `new Date(v).toISOString() === v`로 **왕복이 정확히 일치하는지** 보고, 아니면 거부합니다.

#### 검증을 강화할 때는 반대편도 봐야 합니다

로드 검증을 넣으면서 새 결함을 만들었습니다. **성공한 `POST`가 재시작 불능 파일을 만들었습니다.**

```text
class-validator의 @MaxLength(1000)  →  surrogate pair를 한 자로 센다
내 로더의 String.length             →  UTF-16 코드유닛으로 센다

'😀'.repeat(600)  →  surrogate pair 계수 600 / UTF-16 1200
  DTO:  통과 (201로 저장됨)
  로더: 거부 → 다음 기동에서 부팅 실패
```

같은 계약("최대 1,000자")을 두 곳에서 다르게 해석했고, **엄격한 쪽이 정상 데이터를 막았습니다.** 원래 문제보다 나쁩니다.

정의를 맞추는 것으로 끝내지 않았습니다. `@MaxLength`가 내부적으로 무엇을 세는지는 라이브러리의 구현 사항이고(surrogate pair뿐 아니라 variation selector도 따로 처리합니다), 그쪽이 바뀌면 또 어긋납니다. 그래서 `countCharacters`를 만들어 **DTO와 로더가 같은 함수를 부르게** 했습니다(`@MaxLength` → 직접 만든 `@MaxCharacters`). 이제 일치는 라이브러리 동작과 무관하게 성립합니다.

그리고 불변식을 세웠습니다 — **API가 성공적으로 저장한 데이터는 반드시 다음 기동에서 로드되어야 한다.** ASCII·이모지·한글로 최대 길이까지 채워 `POST` → 재기동 → 조회가 되는지 왕복 테스트로 고정했습니다.
...
7. **손상되거나 스키마가 어긋난 `jobs.json`은 자동으로 고치지 않고 기동을 중단합니다.** 관용적으로 무시하거나 빈 파일로 초기화하면 앱은 뜨지만 데이터를 잃습니다. 사람이 파일을 확인하도록 원본을 보존한 채 비-0 종료 코드로 멈추는 쪽을 택했습니다. 모르는 최상위 키를 거부하는 것도 같은 이유입니다 — 무시하면 다음 쓰기에서 그 키가 조용히 사라집니다.
...
처음에는 이 과제를 producer · jobs · worker의 관계로 파악했습니다([`docs/nestjs-jobs-backend-design.md`](docs/nestjs-jobs-backend-design.md)). Job을 생성하는 다수의 producer와 이를 소비·처리하는 다수의 worker는 일반적으로 쓰이는 설계이며, 죽은 worker의 점유를 회수하는 Reaper, claim, worker별 배치 사이즈, 그리고 반복 실패하는 Job이 처리 슬롯을 잠식하지 않도록 하는 dead-letter와 back-off까지가 Job 처리 시스템의 기본 구조라고 판단했습니다.

명세를 요구사항 단위로 재구성(SDD)하는 과정에서 과제의 의도를 다시 검토했습니다. **이 과제는 Node.js의 주요 특징 — 논블로킹 I/O, 이벤트 루프, 비동기, 싱글 스레드 — 에 대한 이해를 확인하는 것**이고, 이를 가장 간결하게 드러내는 수단으로 로컬 JSON을 두어 그것을 DB처럼 사용할 때 발생하는 문제들을 어떻게 처리하는지 보려는 것으로 이해했습니다.

그런데 DB가 없는 상태에서 다중 프로세스 구조를 유지하면, 결국 **"DB를 만드는 과제"** 가 됩니다. 이는 과제의 목적과 맞지 않고, 규모가 지나치게 커지며 예외 케이스도 계속 늘어납니다. 실제로 파일 잠금 회수의 배타성을 확보하려는 시도는 세 번 모두 실패했습니다.
...
원인은 **이 방식으로는 DB가 제공하는 두 가지를 얻을 수 없다**는 데 있습니다.

① **여러 단계를 묶는 트랜잭션** — 파일시스템에서 원자적인 것은 `rename`·`link`·`O_EXCL` 생성 같은 단일 syscall 하나뿐입니다.

② **세션에 묶인 잠금** — OS 차원에는 존재합니다. POSIX의 `flock`/`fcntl`, Windows의 핸들 기반 byte-range lock은 프로세스나 열린 핸들에 결부되어 종료 시 자동 해제됩니다. 문제는 **Node의 `fs`가 이를 이식 가능한 API로 노출하지 않는다**는 것입니다(`node:fs`에 `flock` 계열이 없습니다). 네이티브 의존성을 두지 않기로 한 제약에서 남는 선택은 `open(path, 'wx')` 기반 잠금 **파일**인데, 이 방식은 소유자가 죽어도 파일이 그대로 남으므로 stale 회수가 필요해집니다.

②를 대체하려면 heartbeat·registry·stale timeout·리더 선출이 필요해지고, ①이 없어 그 회수 절차를 안전하게 만들 수 없습니다. 마지막 시도에서는 정확성을 잠금에서 버전 CAS(`fs.link`로 `versions/v{N+1}.json` 생성)로 옮겼는데, 이는 사실상 MVCC를 직접 구현하기 시작한 것이었습니다.

7차에 걸친 적대적 검증에서 **22건의 결함**이 나왔고, 그 시점에 명세는 755줄·요구사항 90개에 이르렀습니다.

또한 **과제 내용에 다중 프로세스 요구가 명시되어 있지 않습니다.** `여러 프로세스`·`worker`·`producer`·`인스턴스`·`다중`에 해당하는 표현이 나오지 않고, 프로세스 구성에 관한 언급 자체가 없습니다. 명시된 것은 "데이터는 **단일** JSON 파일에 영속화합니다"뿐입니다.

즉 다중 프로세스는 요구사항이 아니라 **제가 추가한 가정**이었고, 프로세스 수는 자유 설계 영역이었습니다. 그렇다면 위에서 본 비용을 감당할 이유가 없습니다.

> 한국어 명사는 수를 표지하지 않으므로, "스케줄러"나 "백그라운드 작업"이 단수형이라는 것 자체는 단일 인스턴스의 근거가 되지 않습니다. 근거는 어디까지나 **다중 프로세스 요구가 없다**는 사실이고, 단일 프로세스는 그 위에서 제가 내린 설계 판단입니다.

**그래서 잠금을 더 정교하게 만드는 대신 간소화하여 문제를 해결하기로 결정했습니다.** writer를 프로세스 하나로 되돌리면 트랜잭션은 인메모리 mutex로, 생존 감지는 기동 복구 한 규칙으로 해결됩니다. 잠금 파일·Reaper·lease·CAS·설정 fingerprint가 모두 사라지고 규칙 다섯 개가 남았습니다.

전환 직전 커밋과 전환 커밋을 같은 기준으로 비교하면 이렇습니다.

| | `docs/SPEC.md` 줄 수 | 요구사항 ID(고유) |
|---|---:|---:|
| 전환 직전 (`5aec756`) | 755 | 90 |
| 전환 (`00c8362`) | 471 | 49 |
| 현재 | 538 | 54 |

> ID 개수는 `grep -oE '\b[A-Z]{2,}-[0-9]+\b' docs/SPEC.md | sort -u | grep -v UTF-16 | wc -l`로 셌습니다. `UTF-16`은 요구사항 ID가 아니므로 제외합니다.

이후 적대적 검증을 반복하며 다시 늘었지만, 늘어난 것은 **영속 데이터 검증과 오류 경계에 관한 규칙**이고 잠금·생존 감지·리더 선출 기계는 돌아오지 않았습니다.

#### 처리량은 실제로 포기했습니다

여기는 정직하게 적어야 하는 부분입니다. 철회한 설계는 **처리량 이득이 있었습니다.**

초기 문서 §13.3의 8단계는 "global lock을 해제한 뒤 실제 작업을 수행한다"입니다. 즉 worker는 짧은 상태 커밋 구간만 직렬화하고, 1분짜리 실제 처리는 잠금 밖에서 합니다. 다른 worker는 이미 잠긴 후보를 건너뛰고 다른 Job을 선점합니다(§13.5). **따라서 N개 worker는 서로 다른 Job N건을 병렬로 처리할 수 있습니다.**

"한 번에 Job 하나"는 **worker당** 제한이지 시스템 전체의 제한이 아니었습니다. 단일 프로세스로 되돌리면서 이 병렬성은 잃었습니다.

바꾼 것은 **정확성과 규모를 위해 처리량을 포기한 트레이드오프**입니다. 과제가 요구한 동작(주기적 상태 점검과 처리, 동시 접근 시 데이터 무결성)은 그대로 만족하고, 처리 단위는 과제가 "자유롭게 가정하여 결정"하라고 명시한 항목입니다. 처리량이 실제로 필요해지는 시점의 답은 아래에 적었습니다 — 파일 위에서 worker를 늘리는 것이 아니라 트랜잭션을 지원하는 저장소로 옮기는 것입니다.

다만 초기 설계가 겨냥했던 요구 자체는 유효합니다. dead-letter와 back-off는 아래 "시간이 더 있다면"에 남겼고, 다중 worker가 실제로 필요해지는 시점의 답도 함께 적었습니다.

### 시간이 더 있다면

1. **수평 확장이 필요해지면 트랜잭션을 지원하는 DB나 MQ 같은 전용 queue로 옮기는 것이 옳습니다.** 파일 잠금을 정교하게 만드는 방향이 아닙니다. `SELECT ... FOR UPDATE SKIP LOCKED` 한 줄이 이 과제의 선점 문제 전체에 해당하고, 트랜잭션과 세션 기반 락이 위에서 실패한 세 시도를 전부 불필요하게 만듭니다.

   그것들을 쓰지 않고 파일 위에서 다중 worker·producer를 구현하려 들면, 결국 **DB를 처음부터 끝까지 직접 만들어보는 과제**가 됩니다. 트랜잭션·락·생존 감지·리더 선출을 차례로 다시 발명하게 되는데, 위에서 22건의 결함으로 확인한 경로가 정확히 그것입니다.
...
└─ jobs/
   ...
   ├─ jobs.types.ts           Job·상태·정렬·메시지 + 영속 데이터 스키마 검증
