...
>
> **[RUN-005]** ③ 로드 시 각 Job 레코드가 [DATA-002]를 만족하는지 검증한다 — UUID v4 `id`, 필수 문자열과 길이, `status` enum, ISO 8601 UTC 시각, 허용된 키만, `id` 중복 없음. 하나라도 어긋나면 손상 파일과 동일하게 `JobsFileLoadError`로 기동을 중단하고, 위반 위치와 사유를 메시지에 담는다(`jobs[1].id가 UUID v4가 아닙니다`).
>
