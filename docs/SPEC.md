...
>
> **[RUN-005]** ③ 로드 시 각 Job 레코드가 [DATA-002]를 만족하는지 검증한다 — UUID v4 `id`, 필수 문자열과 길이, trim된 값, `status` enum, 실재하는 ISO 8601 UTC 시각, 허용된 키만, `id` 중복 없음. 하나라도 어긋나면 손상 파일과 동일하게 `JobsFileLoadError`로 기동을 중단하고, 위반 위치와 사유를 메시지에 담는다(`jobs[1].id가 UUID v4가 아닙니다`).
>
> API 입력은 DTO가 막지만 파일은 사람이 직접 고칠 수 있다. parse만 되고 스키마가 어긋난 레코드(`{"jobs":[{}]}`)를 받아들이면 기동은 성공하고 목록 조회도 `200`을 돌려주지만, 검색에서 `job.title.toLowerCase()`가 `TypeError`를 던져 `500`이 된다 — 원인에서 멀리 떨어진 곳에서 터지므로 로드 시점에 걸러야 한다.

> **[RUN-006]** (DTO와 로더의 경계 일치) **API가 성공적으로 저장한 데이터는 반드시 다음 기동에서 로드되어야 한다.** 두 검증이 같은 규칙을 다르게 판정하면 성공한 `POST`가 재시작 불능 파일을 만든다.
>
> | 규칙 | 정의 |
> |---|---|
> | 길이 | **코드포인트** 수. `String.length`(UTF-16)로 세면 이모지 하나가 2자로 잡혀, DTO를 통과한 값이 로더에서 거부된다 |
> | 공백 | 양쪽 모두 trim된 값만 인정 |
> | 시각 | 형식 검사에 그치지 않고 `new Date(v).toISOString() === v`로 실재 여부 확인. `2026-02-30`은 JS Date가 `2026-03-02`로 넘겨 조용히 다른 값이 된다 |
>
> 길이 판정은 **DTO와 로더가 같은 함수를 공유**해야 한다. 라이브러리의 내부 계수 방식에 의존하면 그쪽이 바뀌는 순간 경계가 어긋난다. API 검증 통과 → 저장 → 재로드 성공을 왕복 테스트로 고정한다([TST-006]).
...
>
> 이 가드는 **기대값을 운영 코드에서 가져오지 않는다.** `JOB_STATUSES`를 그대로 순회하면 운영 코드와 fixture가 같이 잘못됐을 때 테스트도 함께 통과해 oracle 역할을 못 하므로, 세 상태를 테스트 안에 직접 적고 `JOB_STATUSES` 자체가 그 값과 일치하는지도 따로 단언한다. `id`는 [DATA-002]가 UUID v4를 요구하므로 형식만 보는 정규식이 아니라 버전·variant까지 검사한다.

> **[TST-006]** [RUN-006]은 **왕복으로** 검증한다 — DTO 최대 길이로 `POST` → `201` 확인 → 같은 파일에 새 앱 기동 → 그 Job이 조회되는지. ASCII·이모지·한글을 각각 돌려, 코드포인트와 UTF-16이 갈리는 경계를 실제로 지난다. `PATCH` 경로도 같이 본다.
