...
`jobs.json`을 여러 프로세스가 동시에 쓰는 구성은 **의도적으로 지원하지 않는다.** 과제 원문에 다중 프로세스 요구가 명시되어 있지 않으므로 프로세스 수는 자유 설계 영역이며, 단일 프로세스는 그 위에서 내린 판단이다.

OS 차원에는 세션에 묶인 잠금이 있지만(POSIX `flock`/`fcntl`, Windows 핸들 기반 byte-range lock) Node의 `fs`가 이를 이식 가능하게 노출하지 않는다. 네이티브 의존성을 두지 않는 제약에서 남는 선택은 `open(path, 'wx')` 기반 잠금 파일인데, 이 방식은 소유자가 죽어도 파일이 남으므로 stale timeout·소유권 회수·리더 선출이 연쇄적으로 필요해진다. 그 경로는 결국 DB를 직접 구현하는 작업이 되어 과제의 목적과 맞지 않는다([부록 D](#부록-d-다중-프로세스-설계를-철회한-경위)).

수평 확장이 필요해지면 파일 잠금을 정교하게 만드는 것이 아니라 트랜잭션을 지원하는 저장소(`SELECT ... FOR UPDATE SKIP LOCKED`)나 MQ 같은 전용 queue로 이전하는 것이 옳다. 이 판단을 README에 기재한다([DOC-001]).
...
> **[DATA-001]** `jobs.json`의 최상위 키는 `jobs`(배열) 하나다. 로드 시 **정확히 이 하나인지 검사**하고, 다른 키가 있으면 기동을 중단한다([RUN-005]).
>
> 인메모리 상태는 `{ jobs }`만 담으므로, 다른 키가 있는 파일을 받아들이면 다음 쓰기에서 그 키가 **조용히 사라진다.** 손상 파일을 자동 초기화하지 않는 방침([RUN-004])과 정반대되는 데이터 손실이므로, 관용적으로 무시하지 않고 거부한다.
...
>
> **[RUN-005]** ③ 로드 시 파일 전체를 검증한다 — 최상위 키가 정확히 `jobs` 하나([DATA-001]), 그리고 각 Job 레코드가 [DATA-002]를 만족하는지: UUID v4 `id`, 필수 문자열과 길이, trim된 값, `status` enum, 실재하는 ISO 8601 UTC 시각, 허용된 키만, `id` 중복 없음. 하나라도 어긋나면 손상 파일과 동일하게 `JobsFileLoadError`로 기동을 중단하고, 위반 위치와 사유를 메시지에 담는다(`jobs[1].id가 UUID v4가 아닙니다`).
>
...
초기 설계는 이 과제를 producer · jobs · worker의 관계로 파악했다. Job을 생성하는 다수의 producer와 이를 소비·처리하는 다수의 worker는 일반적으로 쓰이는 설계이며, Reaper·claim·worker별 배치 사이즈·dead-letter와 back-off까지가 Job 처리 시스템의 기본 구조라고 판단했다.

명세를 요구사항 단위로 재구성하는 과정에서 과제의 의도를 다시 검토했다. **이 과제는 Node.js의 주요 특징 — 논블로킹 I/O, 이벤트 루프, 비동기, 싱글 스레드 — 에 대한 이해를 확인하는 것**이고, 이를 가장 간결하게 드러내는 수단으로 로컬 JSON을 두어 그것을 DB처럼 사용할 때 발생하는 문제들을 어떻게 처리하는지 보려는 것으로 이해했다.

과제 원문에 다중 프로세스 요구가 명시되어 있지 않다. `여러 프로세스`·`worker`·`producer`·`인스턴스`·`다중`에 해당하는 표현이 나오지 않고, 프로세스 구성에 관한 언급 자체가 없다. 명시된 것은 "데이터는 **단일** JSON 파일에 영속화합니다"뿐이다. 따라서 프로세스 수는 자유 설계 영역이며, 단일 프로세스는 그 위에서 내린 판단이다.

(한국어 명사는 수를 표지하지 않으므로 "스케줄러"가 단수형이라는 것 자체는 단일 인스턴스의 근거가 되지 않는다. 근거는 다중 프로세스 요구가 없다는 사실이다.)

DB가 없는 상태에서 다중 프로세스 구조를 유지하면 결국 **"DB를 만드는 과제"** 가 되어 목적과 맞지 않고, 규모가 커지며 예외 케이스가 계속 늘어난다.

**이 방식으로는 DB가 제공하는 두 가지를 얻을 수 없다.** ① 여러 단계를 묶는 트랜잭션 — 파일시스템에서 원자적인 것은 `rename`·`link`·`O_EXCL` 생성 같은 **단일 syscall 하나**뿐이어서 "읽고 판단하고 쓴다"를 묶을 수단이 없다. ② 세션에 묶인 잠금 — OS 차원에는 존재하지만(POSIX `flock`/`fcntl`, Windows 핸들 기반 byte-range lock) Node의 `fs`가 이식 가능한 API로 노출하지 않는다. 네이티브 의존성 없이 남는 선택은 `open(path, 'wx')` 기반 잠금 **파일**이고, 이 방식은 소유자가 죽어도 파일이 남아 stale 회수가 필요해진다.

②를 대체하려고 heartbeat·registry·stale timeout·리더 선출(Reaper)이 필요해졌고, ①이 없어서 그 회수 절차를 안전하게 만들 수 없었다. 잠금 회수의 배타성을 확보하려는 시도는 세 번 모두 실패했다.
...
공통 원인은 **읽어서 판정한 파일 identity와 나중에 조작하는 pathname을 원자적으로 결속하는 POSIX 연산이 없다**는 것이다. 마지막 판에서는 정확성을 잠금에서 버전 CAS(`fs.link`로 `versions/v{N+1}.json` 생성)로 옮겨 lost update를 막았는데, 이는 사실상 MVCC를 직접 구현하기 시작한 것이었다. 7차에 걸친 적대적 검증에서 22건의 결함이 나왔고, 그 시점에 명세는 755줄·요구사항 90개(커밋 `5aec756`)에 이르렀다. 과제가 요구한 것은 "동시 요청 상황에서도 데이터가 손실되거나 깨지지 않도록 고려해 주세요"이지 분산 합의가 아니다.

그래서 잠금을 더 정교하게 만드는 대신 **간소화하여 문제를 해결하기로 했다.** writer를 프로세스 하나로 되돌리면 트랜잭션은 인메모리 mutex로, 생존 감지는 기동 복구 한 규칙으로 끝난다. 잠금 파일·Reaper·lease·CAS·설정 fingerprint가 전부 사라지고, 남는 것은 [CON-002]·[CON-003]·[CON-005]·[CON-006] 네 규칙이다.

**처리량은 실제로 포기했다.** 초기 설계 §13.3의 8단계는 "global lock을 해제한 뒤 실제 작업을 수행한다"이므로, worker는 짧은 상태 커밋만 직렬화하고 처리는 잠금 밖에서 한다. 다른 worker는 잠긴 후보를 건너뛰고 다른 Job을 선점하므로(§13.5) N개 worker가 서로 다른 Job N건을 병렬 처리할 수 있었다. "한 번에 Job 하나"는 **worker당** 제한이지 시스템 전체의 제한이 아니었다.

단일 프로세스로 되돌리면서 이 병렬성은 잃었다. 정확성과 규모를 위해 처리량을 포기한 트레이드오프이며, 과제가 요구한 동작(주기적 상태 점검과 처리, 동시 접근 시 데이터 무결성)은 그대로 만족한다. 처리 단위는 과제가 "자유롭게 가정하여 결정"하라고 명시한 항목이다.

수평 확장이 필요해지는 시점에는 이 규칙들을 파일 위에서 확장하는 것이 아니라 트랜잭션을 지원하는 저장소나 전용 queue로 이전하는 것이 옳다. `SELECT ... FOR UPDATE SKIP LOCKED` 한 줄이 이 과제의 선점 문제 전체에 해당한다.
