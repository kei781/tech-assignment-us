...
`jobs.json`을 여러 프로세스가 동시에 쓰는 구성은 **의도적으로 지원하지 않는다.** 과제 원문은 다중 프로세스를 요구하지 않으며 문구가 모두 단수다 — "**스케줄러**: `@nestjs/schedule`", "API 요청과 **스케줄러가** 동시에 같은 데이터에 접근할 수 있는 환경", "데이터는 **단일** JSON 파일에 영속화".

파일시스템에는 세션에 묶인 잠금이 없어 프로세스가 죽었는지 알 방법이 없고, 그래서 잠금 파일·stale timeout·소유권 회수·리더 선출이 연쇄적으로 필요해진다. 그 경로는 결국 DB를 직접 구현하는 작업이 되어 과제의 목적과 맞지 않는다([부록 D](#부록-d-다중-프로세스-설계를-철회한-경위)).
...
과제 원문에도 worker나 producer가 다중 프로세스로 동작해야 한다는 언급이 없다. 문구는 모두 단수다 — "**스케줄러**: `@nestjs/schedule`", "API 요청과 **스케줄러가** 동시에 같은 데이터에 접근할 수 있는 환경", "**백그라운드 작업**을 통해 주기적으로 처리", "데이터는 **단일** JSON 파일에 영속화". `여러 프로세스`·`worker`·`producer`·`인스턴스`·`다중`에 해당하는 표현은 나오지 않는다.

DB가 없는 상태에서 다중 프로세스 구조를 유지하면 결국 **"DB를 만드는 과제"** 가 되어 목적과 맞지 않고, 규모가 커지며 예외 케이스가 계속 늘어난다.

**파일시스템에는 DB가 제공하는 두 가지가 없다.** ① 여러 단계를 묶는 트랜잭션 — 원자적인 것은 `rename`·`link`·`O_EXCL` 생성 같은 **단일 syscall 하나**뿐이어서 "읽고 판단하고 쓴다"를 묶을 수단이 없다. ② 세션에 묶인 잠금 — DB는 연결이 끊기면 락을 자동 해제하지만, 파일 잠금은 소유자가 죽었는지 알 방법이 없다.
...
공통 원인은 **읽어서 판정한 파일 identity와 나중에 조작하는 pathname을 원자적으로 결속하는 POSIX 연산이 없다**는 것이다. 마지막 판에서는 정확성을 잠금에서 버전 CAS(`fs.link`로 `versions/v{N+1}.json` 생성)로 옮겨 lost update를 막았는데, 이는 사실상 MVCC를 직접 구현하기 시작한 것이었다. 7차에 걸친 적대적 검증에서 22건의 결함이 나왔고, 그 시점에 명세는 700줄·요구사항 60여 개에 이르렀다. 과제가 요구한 것은 "동시 요청 상황에서도 데이터가 손실되거나 깨지지 않도록 고려해 주세요"이지 분산 합의가 아니다.
...
기능을 축소한 것은 아니다. 철회한 설계도 결국 한 번에 Job 하나를 처리했고, 파일 하나를 공유 저장소로 쓰는 한 병목은 그 파일이었다. 프로세스를 늘려도 실효 처리량은 늘지 않으면서 정확성만 잃는 구조였다.
